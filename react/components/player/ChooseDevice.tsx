...
import { RefreshCcw, LaptopMinimal, Smartphone, Speaker } from 'lucide-react-native';
...
const ChooseDevice = ({ onClose, show, onDeviceSelected }: ChooseDeviceProps) => {
    ...
    <Drawer isOpen={show} onClose={onClose}>
      ...
      <DrawerContent className="w-full max-h-[60vh]">
        <DrawerHeader className="flex-row justify-between items-center">
          <Text style={{ fontWeight: '600', fontSize: 18 }}>Choose a Device</Text>
          <Button variant="link" onPress={() => void getDevice()} className="justify-start">
            <RefreshCcw />
          </Button>
        </DrawerHeader>
        ...
        <DrawerBody>
          <VStack className="gap-3">
            ...
            {!loading && !error && devices.map((device) => {
              ...
              const Icon = () => {
                if (type === 'smartphone') return <Smartphone color={color} width={18} height={18} />;
                if (type === 'computer') return <LaptopMinimal color={color} width={18} height={18} />;
              ...
              return (
                <Button
                  ...
                  variant="link"
                  onPress={() => onDeviceSelected && onDeviceSelected(device.id)}
            ...
            <Button variant="solid" action="secondary" onPress={onClose} className="mt-4 rounded-full">

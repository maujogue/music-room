...
export default function EventDetail() {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: () => (<Event3DotMenu callDelete={() => setShowAlertDialog(true)} callEdit={onEditEvent} />),
    });
  }, [navigation]);
  ...
  return (
    <>
      <VStack className='flex-1'>
        ...
        <EventHeader event={event} expanded={expanded} onToggle={onToggleHeader} />

        {/* Votes / Guests tabs */}
        <Center className='flex-1'>
          <VStack className='px-2 pb-2'>
            <HStack className='w-full bg-primary-500 rounded-xl p-2 justify-between'>
              <TabButton label={'Votes'} isActive={activeTab == 'votes'} onPress={() => setActiveTab('votes')} />
              <TabButton label={'Guests'} isActive={activeTab == 'guests'} onPress={() => setActiveTab('guests')} />
            </HStack>
          </VStack>
          {activeTab == 'votes' ? (

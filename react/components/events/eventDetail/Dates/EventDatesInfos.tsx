...
export default function EventDatesInfos({ event }: Props) {
  ...
  const { start, end, rangeLabel, duration } = useEventDate(event.startDate, event.endDate);
  ...
  return (
    <VStack space="xs">
      ...
      <HStack space="sm" className="pl-4 items-center">
        <Badge size="sm" action="muted" className="rounded-full bg-indigo-200 w-12">
          <BadgeText size='sm' className="capitalize">from</BadgeText>
        </Badge>
        <Text size="xs" className="font-semibold">{start.full}</Text>
      </HStack>
      <HStack space="sm" className="pl-4 items-center">
        <Badge size="sm" action="muted" className="rounded-full bg-indigo-200 w-12">
          <BadgeText size='sm' className="capitalize">to</BadgeText>
        </Badge>
        <Text size="xs" className="font-semibold">{end.full}</Text>
      </HStack>

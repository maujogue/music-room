...
export default function SearchTrack({ playlistId }: Props) {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const res: ApiResponse<SpotifyTrack[]> = await apiFetch<SpotifyTrack[]>(
        `${process.env.EXPO_PUBLIC_SUPABASE_URL}/functions/v1/search?q=${encodeURIComponent(query)}&type=track`
      );

...
export async function fetchCurrentUserPlayingTrack(
  c: Context,
): Promise<Response> {
  ...
  if (!res) {
    throw new HTTPException(500, {
      message: "Failed to fetch Spotify currently playing track",
    });
  }
...
export async function startUserPlayback(c: Context): Promise<Response> {
  ...
  if (res.error) {
    ...
    c.status(res.status || 500);
    return c.json({
      error: res.statusText || "Unknown error from Spotify API",
    });
  }
...
export async function syncSpotifyPlaylists(c: Context): Promise<Response> {
  ...
  if (playlists.error) {
    ...
    return c.json({
      error: playlists.error.message || "Unknown error from Spotify API",
    });
  }
  ...
  if (playlistsPayload.length > 0) {
    ...
    if (error) {
      ...
      throw new HTTPException(500, {
        message: "Failed to upsert playlists into Supabase",
      });
    }

...
Deno.test("READ_EVENT - public event - anyone can read", () => {
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.READ_EVENT, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.READ_EVENT, mockEvent), true);
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.READ_EVENT, mockEvent), true);
});

Deno.test("READ_EVENT - private event - only anonymous can read", () => {
  mockEvent.event.is_private = true;
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.READ_EVENT, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.READ_EVENT, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.READ_EVENT, mockEvent), false);
});

Deno.test("VOTE - everyone can vote", () => {
  mockEvent.event.everyone_can_vote = true;
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.VOTE, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.VOTE, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.VOTE, mockEvent), true);
});

Deno.test("VOTE - restricted vote", () => {
  mockEvent.event.everyone_can_vote = false;
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.VOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.VOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.INVITER, PERMISSIONS.VOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.VOTE, mockEvent), true);
});
...
Deno.test("ADD_USER - public event - anyone can add", () => {
  ...
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.ADD_USER, mockEvent), true);
});
...
Deno.test("ADD_USER - private event - only inviter/owner can add", () => {
  ...
  assertStrictEquals(canUserPerformAction(ROLES.VOTER, PERMISSIONS.ADD_USER, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.INVITER, PERMISSIONS.ADD_USER, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.ADD_USER, mockEvent), true);
});

Deno.test("UNVOTE - everyone can vote => anyone can unvote", () => {
  mockEvent.event.everyone_can_vote = true;
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.UNVOTE, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.UNVOTE, mockEvent), true);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.UNVOTE, mockEvent), true);
});

Deno.test("UNVOTE - restricted vote => only certain roles can unvote", () => {
  mockEvent.event.everyone_can_vote = false;
  assertStrictEquals(canUserPerformAction(null, PERMISSIONS.UNVOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.MEMBER, PERMISSIONS.UNVOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.INVITER, PERMISSIONS.UNVOTE, mockEvent), false);
  assertStrictEquals(canUserPerformAction(ROLES.OWNER, PERMISSIONS.UNVOTE, mockEvent), true);
});

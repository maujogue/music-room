...
async function handleUserInfo(
  ...
  try {
    ...
    if (userDataRes.success && userDataRes.data) {
      const vote_remaining = userDataRes.data.max_votes -
        userDataRes.data.vote_count;
...
async function handleGetEventCurrentTrack(
  userId: string,
  socket: WebSocket,
  message: WebSocketMessage,
): Promise<void> {
  try {
    ...
    if (res.error) {
      sendErrorMessage(
        socket,
        `Failed to fetch owner's currently playing track: ${res.error.message}`,
      );
      return;
    }
    sendSuccessMessage(socket, {
...
function isUnvoteMessage(
  message: WebSocketMessage,
): message is { type: "unvote"; eventId: string; trackId: string } {
  return message.type === "unvote" &&
    typeof message.eventId === "string" &&
    typeof message.trackId === "string";
}

function isGetVoteMessage(
  message: WebSocketMessage,
): message is { type: "vote:get"; eventId: string } {
  return message.type === "vote:get" &&
    typeof message.eventId === "string";
}

function isUserInfoMessage(
  message: WebSocketMessage,
): message is { type: "user:info"; eventId: string } {

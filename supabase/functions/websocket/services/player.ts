...
export async function addItemToSpotifyOwnerQueue(
  item: string,
...
> {
  try {
    ...
    const accessToken = data[0].spotify_access_token as
      | string
      | null
      | undefined;
    ...
    try {
      if (
        eventId && current && current.is_playing &&
        typeof current.progress_ms === "number" && current.item &&
        typeof current.item.duration_ms === "number"
      ) {
        ...
        if (res.data && !res.data.vote_resolved) {
          const voteResult = await resolveVoteCount(
            eventId,
            current,
            accessToken,
            userId,
      ...
      console.warn(
        "Error during leader push in getOwnerCurrentPlayingTrack:",
        err,
      );
    }
...
async function resolveVoteCount(
  ...
  const remainingMs = (current.item?.duration_ms ?? 0) -

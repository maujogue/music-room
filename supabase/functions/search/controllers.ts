...
export async function search(c: Context) {
  ...
  if (type === "track") {
    const new_spotify_token = await refreshSpotifyToken(user.id);
    trackResults = await searchTracks(new_spotify_token, {
      query: q,
      limit,
      offset,
    });
  ...
  if (type === "user" || type === "all") {
    userResults = await searchUsersByQuery(user.id, {
      query: q,
      limit,
      offset,
    });
  ...
  if (type === "event" || type === "all") {
    ...
    eventResults = await Promise.all(
...
async function searchTracks(
  ...
  const res: SpotifyTrackResponse = await fetchSpotifySearch(spotify_token, {
    ...params,
    type: "track",
  });
  if (res.error) {
    throw new HTTPException(res.error.status || 500, {
      message: res.error.message || "Unknown error from Spotify API",
    });
  }
  return res.tracks;
}

...
export async function fetchPlaylistItems(c: Context): Promise<Response> {
  ...
  if (playlist.is_spotify_sync && playlist.spotify_id) {
    ...
    const tracksIds = await fetchSpotifyPlaylistTracksIds(
      playlist,
      spotify_token,
    );
    if (tracksIds) {
  ...
  if (playlist.tracks && playlist.tracks.length !== 0) {
    const trackIds = playlist.tracks.map((track: PlaylistTrack) =>
      track.track_id
    );
    await refreshSpotifyToken(user.id);
    ...
    if (spotifyTracksData.error) {
      ...
      return c.json({
        error: spotifyTracksData.error.message ||
          "Unknown error from Spotify API",
      });
    }

    playlist.tracks = playlist.tracks.map((track: PlaylistTrack) => {
      const spotifyTrack = spotifyTracksData.data.find((
        t: SpotifyTrackDetails,
      ) => t.uri === track.track_id || t.id === track.track_id);
      return {
        ...track,
...
function setUserPlaylistPermissions(
  ...
  if (
    ...
    playlist.collaborators.find((collab: PlaylistCollaborator) =>
      collab.id === user.id
    )
  ) {
  ...
  if (
    playlist.user.role === "owner" || !playlist.is_private ||
    playlist.collaborators.find((collab: PlaylistCollaborator) =>
      collab.id === user.id
    )
  ) {
  ...
  const is_member = playlist.members.find((member: PlaylistMember) =>
    member.id === user.id
  );
  const is_collaborator = playlist.collaborators.find((

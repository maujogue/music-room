...
export function validateCreatePlaylistPayload(
  body: any,
  ...
  if (
    body.is_collaborative !== undefined &&
    typeof body.is_collaborative !== "boolean"
  ) {
    throw new HTTPException(400, {
      message: "is_collaborative must be a boolean",
    });
  }
...
export function validateEditPlaylistPayload(
  body: any,
): Partial<CreatePlaylistPayload> {
  const payload: Partial<CreatePlaylistPayload> = {};

  if (body.name !== undefined) {
    if (
      typeof body.name !== "string" || body.name.length < 3 ||
      body.name.length > 255
    ) {
      throw new HTTPException(400, {
        message: "Name must be a string between 3 and 255 characters",
      });
    }
...
export function validateDeleteTracksPayload(body: any): { uris: string[] } {
  if (
    !body.uris || !Array.isArray(body.uris) ||
    body.uris.some((uri: any) => typeof uri !== "string")
  ) {
    throw new HTTPException(400, {
      message: "uris is required and must be an array of strings",
    });
  }
...
export function validateAddUserPayload(
  body: any,
  ...
  if (!body.role || (body.role !== "member" && body.role !== "collaborator")) {
    throw new HTTPException(400, {
      message: 'role must be either "member" or "collaborator"',
    });
  }
  return { user_id: body.user_id, role: body.role };
}

export function validateRemoveUserPayload(
  body: any,
): { user_id: string; role?: string } {
  if (!body.user_id || typeof body.user_id !== "string") {
    throw new HTTPException(400, {
  ...
  if (body.role && body.role !== "member" && body.role !== "collaborator") {
    throw new HTTPException(400, {
      message: 'role must be either "member" or "collaborator" if provided',
    });
  }
  return { user_id: body.user_id, role: body.role };
}

...
import { assertEquals, assertThrowsAsync } from "https://deno.land/std@0.203.0/testing/asserts.ts";
...
Deno.test('handleSpotifyAuth inserts state and redirects', async () => {
	...
	assertEquals(response.headers.get('Location')?.startsWith('https://accounts.spotify.com/authorize'), true);
});

Deno.test('handleSpotifyCallback returns 400 if state is invalid', async () => {
	const req = new Request('http://localhost/spotify/callback?code=abc&state=invalid');
	const res = new Response();
